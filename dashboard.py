...
def load_model():
    try:
        ...
        st.error("Model file not found. Please ensure 'complete_equipment_failure_prediction_system.pkl' exists.")
        return None
...
def load_data():
    try:
        ...
        # Ensure failure_probability column exists
        ...
        st.error("Data file not found. Please ensure 'cleaned_equipment_data.csv' exists.")
        return None
    except Exception as e:
...
def render_all_equipment_view():
    ...
    if model_system:
        ...
        # Use existing failure probability if available, otherwise predict
...
def main_dashboard():
    ...
    # Metrics section removed for cleaner interface
...
def main():
    ...
    if current_view == 'dashboard':
        ...
        try:
            ...
            st.error("Database dashboard module not available")
        except Exception as e:
            st.error(f"Error loading database dashboard: {e}")
            st.info("Please run database migration first: python database_migration.py")
        rendered = True
        ...
        try:
            ...
            if equipment_data is not None:
                ...
                equipment_data['health_score'] = equipment_data.apply(calculate_health_score, axis=1)
                render_advanced_analytics_page(equipment_data)
            else:
                st.error("Unable to load equipment data for analytics")
        except ImportError:
